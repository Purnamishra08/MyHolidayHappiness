...
class Itinerary extends CI_Controller {
	...
	{	
		...
		$itineraryurl = $this->uri->segment(2);
		if(!empty($itineraryurl))
		{	
			$itineraryurl = $this->db->escape($itineraryurl);
			if(isset($_REQUEST['preview']))
			{				
				$de_admin = $this->Common_model->decode($_REQUEST['preview']);
				if($de_admin=="admin")
				{
					$validitinerary = $this->Common_model->noof_records("itinerary_id","tbl_itinerary","itinerary_url=$itineraryurl");
					$data['itinerary_data']=$this->Common_model->get_records("*","tbl_itinerary","itinerary_url=$itineraryurl");
				}
				else
			...
			{
				$validitinerary = $this->Common_model->noof_records("itinerary_id","tbl_itinerary","itinerary_url=$itineraryurl and status='1'");
				$data['itinerary_data']=$this->Common_model->get_records("*","tbl_itinerary","itinerary_url=$itineraryurl and status='1'");
			}
	...
	public function search()
	{
		...
		$tour_package_destids = $_REQUEST["destination_id"];
		?>
		<div id="loader"></div>		
		<?php
		$condition = "";
		...
		$tour_packages = $this->Common_model->get_records("*", "tbl_tourpackages", "itinerary in (SELECT DISTINCT(itinerary_id) FROM tbl_itinerary_destination WHERE destination_id in ($tour_package_destids)) and `status` = 1 $condition","");
