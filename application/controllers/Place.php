...
class Place extends CI_Controller {
	...
	{	
		$desturl = $this->uri->segment(3);
		$placeurl = $this->uri->segment(4);
		if(!empty($placeurl))
		{	
			...
			{				
				...
				if($de_admin=="admin")
				{							
					$validplace = $this->Common_model->noof_records("placeid","tbl_places","place_url=$placeurl");
					$data['placedata']=$this->Common_model->get_records("*","tbl_places","place_url=$placeurl");
				}
				else
			...
			{
				// check destination with place
				$destination_id = $this->Common_model->showname_fromid("destination_id", "tbl_destination", "destination_url=$desturl");

				$validplace = $this->Common_model->noof_records("placeid","tbl_places","place_url=$placeurl and destination_id='$destination_id' and status='1'");
				$data['placedata']=$this->Common_model->get_records("*","tbl_places","place_url=$placeurl and destination_id='$destination_id' and status='1'");
			}	
	...
	public function search()
	{
		...
		$tour_packages = $this->Common_model->get_records("*", "tbl_tourpackages", "itinerary in (SELECT DISTINCT(itinerary_id) FROM tbl_itinerary_daywise WHERE place_id ='$placeid' or place_id like '$placeid,%' or place_id like '%,$placeid' or place_id like '%,$placeid,%') and status=1 $condition","");
