...
/*    public function pdf_generator(){
    ...
    public function add() {
        ...
        if (isset($_POST['btnSubmitPlace']) && !empty($_POST)) {
            ...
            $this->form_validation->set_rules('alttag_banner', 'Alt Tag', 'trim|required|xss_clean');
            $this->form_validation->set_rules('alttag_thumb', 'Alt Tag', 'trim|required|xss_clean');
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($noof_duprec < 1) {
                    ...
                    $tourimofilename = $this->Common_model->seo_friendly_url($alttag_banner);
                    $tourthumbfilename = $this->Common_model->seo_friendly_url($alttag_thumb);
                    if (isset($_FILES['tourimo']) && $_FILES['tourimo']['name'] != '') {
                        $filename = $this->Common_model->ddoo_upload('tourimo', 745 , 450, $tourimofilename);
                    } else {
                        $filename = NULL;
                    } 
                    
                    if (isset($_FILES['tourthumb']) && $_FILES['tourthumb']['name'] != '') {
                        $tourthumb_imgnew = $this->Common_model->thumbddoo_upload('tourthumb', 300 , 225, $tourthumbfilename);
                    } else {
                ...
                ///Start impage upload
                    ...
                    if (isset($_FILES['tourimo']) && $_FILES['tourimo']['name'] != '') {
                        ...
                        if(file_exists($oldname_b)) {
                            ...
                            $newname_b = getcwd().'/uploads/'.$tourimofilename.'.'.$extensionB;
                            rename($oldname_b, $newname_b);
                            $filename = $tourimofilename.'.'.$extensionB;
                        } else {
                            $filename = $rimage;
                        }
                    }                 
                    ...
                    if (isset($_FILES['tourthumb']) && $_FILES['tourthumb']['name'] != '') {
                        ...
                        $thumb_imgnew = $this->Common_model->thumbddoo_upload('tourthumb', 300 , 225, $tourthumbfilename);
                    } else {
                        $oldname = getcwd().'/uploads/'.$rimagethumb;
                        if(file_exists($oldname)) {
                            $extensionArr = explode(".", $rimagethumb);
                            $extension = end($extensionArr);
                            $newname = getcwd().'/uploads/'.$tourthumbfilename.'.'.$extension;
                            rename($oldname, $newname);
                            $thumb_imgnew = $tourthumbfilename.'.'.$extension;
                        } else {
