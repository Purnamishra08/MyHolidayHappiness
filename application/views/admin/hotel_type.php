                        ...
                        <div class="col-sm-4">
                            <div  style="margin-bottom: 25px;box-shadow: 0 3px 6px rgba(0, 0, 0, 0.16);
                                ...
                                <!-- <form name="colour" id="colour" method="post"> -->
                                    ...
                                    <?php echo form_open('', array( 'id' => 'form_hotel_type', 'name' => 'form_hotel_type'));?>
                                    <?php echo $message; ?>
                                    <div class="form-group">
                                        <label>Hotel Type</label>
                                        <input type="text"  class="form-control" placeholder="Enter Hotel type" name="hotel_type_name" id="hotel_type_name" value="<?php echo set_value('hotel_type_name'); ?>">
                                    </div>
                                            ...
                                            <table id="example1" class="table table-bordered table-striped table-hover">
                                                ...
                                                <tbody>
                                                    ...
                                                    if (!empty($row)) {
                                                        foreach ($row as $rows) {
                                                            $cnt++;
                                                            $hotel_type_id = $rows['hotel_type_id'];
                                                            $hotel_type_name = $rows['hotel_type_name'];
                                                            $status = $rows['status'];
                                                            ?>
                                                            <tr>
                                                                <td><?php echo $cnt; ?></td>
                                                                <td><?php echo $hotel_type_name; ?></td>
                                                        
                                                                <td><!-- <span class="label-custom label label-default">Active</span> -->
                                                                    <?php if ($status == 1) { ?>
                                                                        <span class="status" data-id="<?php echo "status-" . $hotel_type_id; ?>"><a href="javascript:void(0)" title="Status is active. Click here to make it inactive."><span class="label-custom label label-success">Active</span></a></span>
                                                                    <?php } else { ?>
                                                                        <span class="status" data-id="<?php echo "status-" . $hotel_type_id; ?>"><a href="javascript:void(0)" title="Status is inactive. Click here to make it active."><span class="label-custom label label-danger">Inactive</span></a></span>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <a href="javascript:void(0);" data-id="<?php echo $hotel_type_id; ?>" data-toggle="modal" data-target="#myModal" class="btn btn-primary btn-sm view" title="View"><i class="fa fa-pencil"></i></a>
                                                                    <!-- <button type="button" class="btn btn-danger btn-sm">
                                                                        <i class="fa fa-trash-o"></i> </button> -->
                                                                    <a onClick="return confirm('Are you sure to delete this hotel type?')" href="<?php echo base_url() . 'admin/hotel_type/delete/' . $hotel_type_id; ?>" class="btn btn-danger btn-sm" title="Delete"><i class="fa fa-trash-o"></i> </a>
                                                                </td>
                                                            </tr>
    ...
    <script src="<?php echo base_url(); ?>assets/admin/js_validation/validation.js"></script>
            <script type="text/javascript">
                ...
                $(document).on('click', '.view', function () {
                    ...
                    $.ajax({
                        url: "<?php echo base_url(); ?>admin/hotel_type/edit_pop/" + val,
                        type: 'post',
                ...
                $(document).on('click', '.status', function () {
                    if (confirm('Are you sure to change the status?'))
                    {
                        ...
                        $.ajax({
                            url: "<?php echo base_url(); ?>admin/hotel_type/changestatus/" + id,
                            type: 'post',
                            data: {'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'},
                            cache: false,
                            //processData: false,
