...
<html lang="en">
            ...
            <div class="content-wrapper">
                <section class="content-header">
                    ...
                    <div class="header-title">
                        <h1>Vehicle type</h1>
                        <small>Manage vehicle type</small>
                    </div>
                </section>
                <!-- Main content -->
                        ...
                        <div class="col-sm-4">
                            <div  style="margin-bottom: 25px;box-shadow: 0 3px 6px rgba(0, 0, 0, 0.16);
                                ...
                                <!-- <form name="colour" id="colour" method="post"> -->
                                   ...
                                   <?php echo form_open('', array( 'id' => 'form_vehicletype', 'name' => 'form_vehicletype'));?>
                                    <?php echo $addmessage; ?>
                                    
                                    <div class="form-group">
                                        <label> Vehicle name </label>
                                        <input type="text" class="form-control" placeholder="Enter vehicle name" name="vehicle_name" id="vehicle_name" value="<?php echo set_value('vehicle_name'); ?>">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" class="form-control" placeholder="Enter vehicle capacity" name="capacity" id="capacity" value="<?php echo set_value('capacity'); ?>">

                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="reset-button">  
                                        <button type="submit" class="btn redbtn" name="btnSubmitvehicle" id="btnSubmitvehicle">Save</button>
                                            ...
                                            <table id="example1" class="table table-bordered table-striped table-hover">
                                                <thead>
                                                    ...
                                                    if (!empty($row)) {
                                                        ...
                                                        <td><?php echo $rows['capacity'].' Members'; ?></td>

                                                        <td style="text-align: center;">				                                        
			                                            <?php if($status==1) { ?>
			                                                <span class="status" data-id="<?php echo "status-".$vehicleid; ?>"><a href="javascript:void(0)" title="Status is active. Click here to make it inactive."><span class="label-custom label label-success">Active</span></a></span>
			                                            <?php } else { ?>
			                                            	<span class="status" data-id="<?php echo "status-".$vehicleid; ?>"><a href="javascript:void(0)" title="Status is inactive. Click here to make it active."><span class="label-custom label label-danger">Inactive</span></a></span>
			                                            <?php } ?>

				                                        </td>     
                                                        
                                                    <td>
														 <a href="javascript:void(0);" data-id="<?php echo $vehicleid; ?>" data-toggle="modal" data-target="#myModal" class="btn btn-primary btn-sm view" title="View"><i class="fa fa-pencil"></i></a>
														
														<a onClick="return confirm('Are you sure to delete this?')" href="<?php echo base_url().'admin/vehicletype/delete/'.$vehicleid; ?>" class="btn btn-danger btn-sm" title=""><i class="fa fa-trash-o"></i> </a>
													</td>
    ...
    <script src="<?php echo base_url(); ?>assets/admin/js_validation/validation.js"></script>
            <script type="text/javascript">
                ...
                $(document).on('click', '.view', function () {
                    ...
                    $.ajax({
                        url: "<?php echo base_url(); ?>admin/vehicletype/vtypeeditpopup/" + val,
                        type: 'post',
    ...
    $(document).on('click', '.status', function(){
				if(confirm('Are you sure to change the status?'))
				{
					...
					$.ajax({
						url: "<?php echo base_url(); ?>admin/vehicletype/changestatus/"+id,
						type: 'post',
						data: {'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'},
						cache: false,
