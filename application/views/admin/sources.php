                        ...
                        <div class="col-sm-4">
                            <div  style="margin-bottom: 25px;box-shadow: 0 3px 6px rgba(0, 0, 0, 0.16);
                                ...
                                <!-- <form name="colour" id="colour" method="post"> -->
                                    ...
                                    <?php echo form_open('', array( 'id' => 'form_source', 'name' => 'form_source'));?>
                                    <?php echo $message; ?>
                                    <div class="form-group">
                                        <label>Source</label>
                                        <input type="text"  class="form-control" placeholder="Enter Source" name="source_name" id="source_name" value="<?php echo set_value('source_name'); ?>">
                                    </div>
                                                    ...
                                                    if (!empty($row)) {
                                                        foreach ($row as $rows) {
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <?php if ($status == 1) { ?>
                                                                        <span class="status" data-id="<?php echo "status-" . $source_id; ?>"><a href="javascript:void(0)" title="Status is active. Click here to make it inactive."><span class="label-custom label label-success">Active</span></a></span>
                                                                    <?php } else { ?>
                                                                        <span class="status" data-id="<?php echo "status-" . $source_id; ?>"><a href="javascript:void(0)" title="Status is inactive. Click here to make it active."><span class="label-custom label label-danger">Inactive</span></a></span>
                                                                    <?php } ?>
                                                                </td>
                                                                <td>
                                                                    <a href="javascript:void(0);" data-id="<?php echo $source_id; ?>" data-toggle="modal" data-target="#myModal" class="btn btn-primary btn-sm view" title="View"><i class="fa fa-pencil"></i></a>
                                                                    <a onClick="return confirm('Are you sure to delete this source?')" href="<?php echo base_url() . 'admin/sources/delete/' . $source_id; ?>" class="btn btn-danger btn-sm" title="Delete"><i class="fa fa-trash-o"></i> </a>
                                                                </td>
                                                            </tr>
                                                            <?php
                                                        }
    ...
    <script src="<?php echo base_url(); ?>assets/admin/js_validation/validation.js"></script>
            <script type="text/javascript">
                ...
                $(document).on('click', '.view', function () {
                    ...
                    $.ajax({
                        url: "<?php echo base_url(); ?>admin/sources/edit_pop/" + val,
                        type: 'post',
                ...
                $(document).on('click', '.status', function () {
                    if (confirm('Are you sure to change the status?'))
                    {
                        ...
                        $.ajax({
                            url: "<?php echo base_url(); ?>admin/sources/changestatus/" + id,
                            type: 'post',
                            data: {'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'},
                            cache: false,
                            //processData: false,
