        ...
        <section class=" mb100">
            <div class="container">
                <ul class="cbreadcrumb my-3">
                  ...
                  <?php if (!empty($cat_name)) { ?> <li><a href="#"><?php echo $cat_name; ?></a></li><?php }?>
                </ul>
                <div class="row sec-right-left">
                    ...
                    <div class="col-xl-8 col-lg-8 sec-right">
						<h1 class="mb-2 bluefont"><?php if (!empty($cat_name)) { ?><?php echo $cat_name; ?><?php }else{?> Getaways <?php }?></h1>
                        <div class="getwaybg">	
							<?php
							    if(sizeOf($getaway_id)>0){
							      $noof_getaway_dests = $this->Common_model->noof_records("a.tag_id","tbl_tags as a, tbl_destination as b","a.type_id=b.destination_id and a.tagid in (".implode(',',$getaway_id).") and a.type=1 and b.status=1"); 
								  
							    }else{
							        $noof_getaway_dests = $this->Common_model->noof_records("a.tag_id","tbl_tags as a, tbl_destination as b","a.type_id=b.destination_id and a.type=1 and b.status=1"); 
								...
								if($noof_getaway_dests>0)
								{
									$count = 1;
									 if(sizeOf($getaway_id)>0){
									$getaway_dests = $this->Common_model->join_records("a.*, b.destination_url, b.destination_name, b.destiimg_thumb, b.alttag_thumb, b.state, b.about_destination","tbl_tags as a","tbl_destination as b", "a.type_id=b.destination_id", "a.tagid in (".implode(',',$getaway_id).") and a.type=1 and b.status=1","b.destination_name asc");
									 }else{
									 	$getaway_dests = $this->Common_model->join_records("a.*, b.destination_url, b.destination_name, b.destiimg_thumb, b.alttag_thumb, b.state, b.about_destination","tbl_tags as a","tbl_destination as b", "a.type_id=b.destination_id", " a.type=1 and b.status=1","b.destination_name asc");
							...
							<?php
							if(sizeOf($getaway_id)>0){
								$noof_getaway_places = $this->Common_model->noof_records("a.tag_id","tbl_tags as a, tbl_places as b"," a.type_id=b.placeid and a.tagid in (".implode(',',$getaway_id).") and a.type=2 and b.status=1"); 
							}else{
							   $noof_getaway_places = $this->Common_model->noof_records("a.tag_id","tbl_tags as a, tbl_places as b"," a.type_id=b.placeid and a.type=2 and b.status=1"); 
							...
							<hr>						
								<?php
									$count1 = 1;
									 if(sizeOf($getaway_id)>0){
									$getaway_places = $this->Common_model->join_records("a.*, b.destination_id, b.place_name, b.place_url, b.placethumbimg, b.alttag_thumb, b.about_place","tbl_tags as a","tbl_places as b", "a.type_id=b.placeid", "a.tagid in (".implode(',',$getaway_id).") and a.type=2 and b.status=1","b.place_name asc");
								}else{	$getaway_places = $this->Common_model->join_records("a.*, b.destination_id, b.place_name, b.place_url, b.placethumbimg, b.alttag_thumb, b.about_place","tbl_tags as a","tbl_places as b", "a.type_id=b.placeid", "a.type=2 and b.status=1","b.place_name asc");
									}
                        ...
                        <div class="getway-sec">
                            <?php
                             if(sizeOf($getaway_id)>0){
								$popular_getaways = $this->Common_model->join_records("a.tag_name, a.tag_url, b.cat_name", "tbl_menutags as a", "tbl_menucateories as b","a.cat_id=b.catid", "a.menuid=1 and a.status=1 and tagid not in (".implode(',',$getaway_id).")","RAND()","10");
							}else{
							    $popular_getaways = $this->Common_model->join_records("a.tag_name, a.tag_url, b.cat_name", "tbl_menutags as a", "tbl_menucateories as b","a.cat_id=b.catid", "a.menuid=1 and a.status=1 ","RAND()","10");
